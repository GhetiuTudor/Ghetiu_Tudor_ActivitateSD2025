...
void adaugaMasinaInVector(Masina** vct, int* nr, Masina mNoua)
{
    Masina* aux;
    aux=malloc(sizeof(Masina)*((*nr)+1));
    for(int i=0;i<(*nr);i++)
    {
        aux[i]=(*vct)[i];
    }
...
Masina citireDinFisier(FILE* file)
{
    ...
    m.id=atoi(aux);
    m.anFabricatie=atoi(strtok(NULL,delim));
    aux=strtok(NULL,delim);
    m.sofer=malloc(strlen(aux)+1);
    strcpy(m.sofer, aux);
    m.kilometriiParcursi=atof(strtok(NULL, delim));
    aux=strtok(NULL,delim);
    m.initialaProducator=aux[0];
    return m;
}

Masina* citireVectorDinFisier(const char* numeFisier, int* nrMasini)
{
    FILE* g=fopen(numeFisier, "r");
    Masina* vct=NULL;
    while(!feof(g))
    {
        Masina m=citireDinFisier(g);
        adaugaMasinaInVector(&vct,nrMasini,m);
    }
    fclose(g);
    return vct;
}
...
int main()
{
    ...
    //FISIERE
    int nr=3;
    adaugaMasinaInVector(&vector, &nr, m4);
    //afisareVector(vector,nr);
    FILE* f=fopen("masina.txt", "r");
    Masina m5= citireDinFisier(f);
    //afisare(m5);
    int nrCitit=3;
    //Masina* vctCitit=citireVectorDinFisier("masini.txt", &nrCitit);
    //afisareVector(vctCitit,3);
